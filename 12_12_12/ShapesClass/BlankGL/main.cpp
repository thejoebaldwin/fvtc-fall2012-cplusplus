...
rectangle* allSquares[squareLength];
...
void draw(GLenum mode)
{
	...
	for (int i = 0; i < squareLength; i++)
	{
		...
		if (allSquares[i]->getType() == "square")
		{
			square* s = (square*)  allSquares[i];
			s->draw();
		}
		else
		{
			rectangle* r = (rectangle*)  allSquares[i];
			r->draw();
		}
...
{
	//any code here for when an object was selected
